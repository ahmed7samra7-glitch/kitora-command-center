...
import { getConfirmedWhatsAppEvidence, recordWhatsAppDeliveryEvidence, verifyWhatsAppWebhookSignature } from '../server/whatsappDeliveryEvidence.js';

async function main() {
  const previousSecret = process.env.META_WHATSAPP_APP_SECRET;
  process.env.META_WHATSAPP_APP_SECRET = 'verification-secret';

  const rawBody = JSON.stringify({ entry: [{ changes: [{ value: { statuses: [{ id: 'wamid.VERIFY', status: 'delivered', recipient_id: '15550001111', timestamp: '1760000000' }] } } }] });
  const signature = `sha256=${crypto.createHmac('sha256', process.env.META_WHATSAPP_APP_SECRET).update(rawBody, 'utf8').digest('hex')}`;
