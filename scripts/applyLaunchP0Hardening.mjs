...
// 1) Storage: fail closed in production when local JSON is selected.
let db = read(dbPath);
db = replaceOnce(
  db,
...
db = replaceOnce(
  ...
  'storage status metadata'
);
write(dbPath, db);

// 2) Mission loop: explicit bounded tick + production-safe loop flag.
...
// 3) Agent runtime: add bounded public batch API without changing existing business handlers.
...
// 5) Explicit configuration documentation.
