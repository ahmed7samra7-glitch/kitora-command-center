...
export default {
  async fetch(request: Request, env: KccCloudflareEnv): Promise<Response> {
    ...
    if (request.method === 'GET' && url.pathname === '/api/kcc/runtime') {
      return json({ success: true, runtime: 'cloudflare-workers', storage: 'cloudflare-d1', backgroundExecution: 'event-driven-only', localFilesystemPersistence: false, failClosed: true });
    }
    ...
    if (request.method === 'POST' && url.pathname === '/api/kcc/tasks') {
      ...
      const task = await enqueueCloudflareTask(env.KCC_DB, String(body.type || 'KCC_TASK'), body.payload || {});
